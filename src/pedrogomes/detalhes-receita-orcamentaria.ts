...
import z from "zod";
import type { DetalhesReceitaOrcamentariaSchemaType } from "../../http-schemas/DetalhesReceitaOrcamentaria";
import { BASE_URL } from "../shared/constants";

const kat = Katalist();

export const GetDetalhesReceitaOrcamentariaSearchSchema = z.object({
	diaInicio: z
		.string()
		.describe(
	...
	codigochave: z
		.string()
		.describe(
			"Código da receita para buscar detalhes (formato: '1112.50.0.1', etc.). Exemplo: '1112.50.0.1'.",
		),
...
export async function detalhesReceitaOrcamentaria({
	diaInicio,
	mesInicio,
	diaFinal,
	mesFinal,
...
}: z.infer<typeof GetDetalhesReceitaOrcamentariaSearchSchema>) {
	...
	receitaParams.set("Listagem", "DetalhesReceitaOrcamentaria");
	...
	receitaParams.set("Codigochave", codigochave);
	receitaParams.set(
		"MostraDadosConsolidado",
		mostraDadosConsolidado.toString(),
	);

	const receita = await kat.get<DetalhesReceitaOrcamentariaSchemaType>(
